...
function parseFrontmatter(source: string): { meta: Record<string, unknown>; body: string } {
  ...
  for (const rawLine of match[1].split(/\r?\n/)) {
    ...
    if (val === "|") { inExecuteBlock = true; continue; }
    if (val === "")  { currentList = []; meta[currentKey] = currentList; }
    else             { meta[currentKey] = val.replace(/^['"]|['"]$/g, ""); }
  }

  if (inExecuteBlock) meta["execute"] = execLines.join("\n").trimEnd();
  return { meta, body };
}

function buildMeta(raw: Record<string, unknown>, filePath: string): SkillMeta {
  return {
...
export class SkillLoader {
  ...
  resolve(nameOrTrigger: string): Skill | undefined {
  ...
  registerFromSource(source: string, domain = "input"): Skill {
    ...
    const skill: Skill = { meta, body, filePath };
  ...
  delete(name: string): boolean {
    ...
    try { fs.unlinkSync(skill.filePath); } catch { /* already gone */ }
  ...
  // Public — invoke
  // ------------------------------------------------------------------
  async invoke(nameOrTrigger: string, args: Record<string, unknown>, wm: WindowManager): Promise<InvokeResult> {
    const skill = this.resolve(nameOrTrigger);
    if (!skill) return { status: "error", error: `Skill not found: ${nameOrTrigger}` };
  ...
  // Public — hot memory
  // ------------------------------------------------------------------
  readHotMemory(): string {
    if (!fs.existsSync(this.memoryFile)) return "<!-- memory.md not found -->";
    return fs.readFileSync(this.memoryFile, "utf-8");
  }

  writeHotMemory(content: string): { ok: boolean; lines: number; error?: string } {
  ...
  readHeartbeat(): HeartbeatState {
    ...
    const state: Partial<HeartbeatState> = {};
    for (const line of text.split(/\r?\n/)) {
      const m = line.match(/^(\w+):\s*(.*)$/);
      if (m) (state as Record<string, unknown>)[m[1]] = m[2] === "null" ? null : m[2];
    }
    return state as HeartbeatState;
  }

  updateHeartbeat(patch: Partial<HeartbeatState>): void {
  ...
  // ------------------------------------------------------------------
  logCorrection(entry: string): void {
  ...
  readCorrections(): string {
    if (!fs.existsSync(this.correctionsFile)) return "<!-- no corrections yet -->";
  ...
  // Public — hot ↔ cold
  // ------------------------------------------------------------------
  decaySkill(name: string): { ok: boolean; error?: string } {
    const skill = this.skills.get(name);
  ...
  promoteSkill(name: string): { ok: boolean; error?: string } {
    const archivePath = path.join(this.archiveDir, `${name}.md`);
    if (!fs.existsSync(archivePath)) return { ok: false, error: `Not in archive: ${name}` };
  ...
  stop(): void { for (const w of this.watchers.values()) w.close(); }
  ...
  private _watchAll(): void {
    ...
    for (const dir of dirs) {
      ...
      const w = fs.watch(dir, (_evt, filename) => {
        ...
        if (["memory.md", "index.md", "heartbeat-state.md", "corrections.md"].includes(filename)) return;
        ...
        if (fs.existsSync(full)) {
          ...
          for (const [name, s] of this.skills) {
            if (s.filePath === full) { this.skills.delete(name); this.rebuildIndex(); }
          }
        }
      });
