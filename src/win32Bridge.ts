...
// Typed helpers
// ---------------------------------------------------------------------------
export async function findWindow(titleContains: string): Promise<string> {
  return runPS(`[Win32]::FindByTitle(${JSON.stringify(titleContains)})`);
}

export async function postMessage(
  hwndHex: string, msg: number, wp: number, lp: bigint | number
): Promise<void> {
  const lpVal = typeof lp === "bigint" ? lp.toString() : lp;
  await runPS(
    `[Win32]::PostMessage([IntPtr]${hwndHex}, 0x${msg.toString(16)}, [IntPtr]${wp}, [IntPtr]${lpVal})`
  );
}

export async function screenshot(hwndHex: string): Promise<string> {
  return runPS(`[Win32]::Screenshot([IntPtr]${hwndHex})`);
}

export async function getWindowRect(hwndHex: string): Promise<{left:number,top:number,right:number,bottom:number}|null> {
