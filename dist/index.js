...
const STATIC_TOOLS = [
    {
        ...
        description: "Open a dedicated browser/GUI window for the agent. " +
    ...
    {
        name: "agent_key",
        description: "Send a virtual-key press. Common codes: Enter=13, Backspace=8, " +
    ...
    {
        name: "agent_execute",
        description: "Execute an arbitrary JavaScript snippet against the WindowManager. " +
    ...
    },
    {
        ...
        description: "Execute an arbitrary PowerShell snippet inside the workspace context. " +
    ...
    },
    {
        ...
        description: "Dynamically register a new MCP tool at runtime by providing a CommonJS " +
    ...
    {
        name: "list_skills",
        description: "List every skill available at runtime (name, description, triggers, " +
    ...
    {
        name: "load_skill",
        description: "Return the full content of a skill: frontmatter metadata, markdown " +
    ...
    {
        name: "register_skill",
        description: "Create or update a skill by providing its full markdown source " +
...
// Server setup
// ---------------------------------------------------------------------------
const server = new Server({ name: "agent-isolated-input", version: "1.0.0" }, { capabilities: { tools: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    return { content: await dispatch(name, args) };
});
// ---------------------------------------------------------------------------
// Dispatcher
// ---------------------------------------------------------------------------
async function dispatch(name, args) {
    ...
    switch (name) {
        case "launch_agent_workspace":
            return ok(await wm.launchWorkspace(args.url ?? "about:blank"));
        ...
        case "agent_click":
            return ok(await wm.click(args.x, args.y, args.button ?? "left"));
        case "agent_double_click":
            return ok(await wm.doubleClick(args.x, args.y));
        case "agent_move_mouse":
            return ok(await wm.moveMouse(args.x, args.y));
        case "agent_scroll":
            return ok(await wm.scroll(args.x, args.y, args.direction, args.amount ?? 3));
        case "agent_type":
            return ok(await wm.typeText(args.text));
        case "agent_key":
            return ok(await wm.keyPress(args.vk_code));
        case "agent_navigate":
            return ok(await wm.navigate(args.url));
        ...
        case "agent_execute_ps":
            return ok(await wm.executePS(args.script));
        case "agent_register_tool": {
            try {
                const tool = loader.registerInline(args.source);
        ...
        case "load_skill": {
            const skill = skillLoader.resolve(args.name);
            if (!skill)
                return ok({ error: `Skill not found: ${args.name}` });
            return ok({
        ...
        case "invoke_skill": {
            ...
            const result = await skillLoader.invoke(args.name, skillArgs, wm);
            return ok(result);
        }
        case "register_skill": {
            try {
                const skill = skillLoader.registerFromSource(args.source);
        ...
        case "delete_skill": {
            const deleted = skillLoader.delete(args.name);
        ...
        case "memory_hot_write":
            return ok(skillLoader.writeHotMemory(args.content));
        case "memory_heartbeat":
            return ok(skillLoader.readHeartbeat());
        case "memory_heartbeat_update":
            skillLoader.updateHeartbeat(args);
            return ok({ status: "ok" });
        case "memory_log_correction":
            skillLoader.logCorrection(args.entry);
            return ok({ status: "ok" });
        case "memory_corrections":
            return ok({ content: skillLoader.readCorrections() });
        case "skill_decay":
            return ok(skillLoader.decaySkill(args.name));
        case "skill_promote":
            return ok(skillLoader.promoteSkill(args.name));
