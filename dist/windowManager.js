...
export class WindowManager {
    ...
    // Launch
    // ------------------------------------------------------------------
    async launchWorkspace(url = "about:blank") {
    ...
    async executePS(script) {
        try {
            const result = await runPS(`$hwnd = [IntPtr]${this.hwnd ?? "0"}\n${script}`);
    ...
    async getStatus() {
        await this._refreshHandle();
        const base = { active: !!this.hwnd, hwnd: this.hwnd, win32Available: true };
        if (this.hwnd) {
